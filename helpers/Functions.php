...
function set_img_src($imgsrc, $width = null, $height = null, $returnindex = 0)
{
    if (!empty($imgsrc)) {
        ...
        // Retorna una ruta relativa completa
        $url = "/public_clinic_app/" . ltrim($src, "/");
...
{
    $post = filter_input_array(INPUT_POST); // Sin filtro directo (puedes filtrar por campo si deseas)

    if (!empty($post[$field])) {
        // Sanitización explícita
        return htmlspecialchars(trim($post[$field]), ENT_QUOTES, 'UTF-8');
    } else {
        return $default_value;
    }
}
